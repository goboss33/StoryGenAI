...
interface Props {
  ...
  onNext: () => void;
  onImport?: (state: StoryState) => void;
  isDebugMode: boolean;
...
const Step1Idea: React.FC<Props> = ({ 
    ...
    onUpdate, onNext, onImport, isDebugMode, setIsDebugMode
}) => {
  ...
  return (
    <div className="flex flex-col h-full relative">
      {/* Top Right Actions: Import & Debug Toggle */}
      <div className="absolute top-0 right-0 z-20 flex items-center gap-3">
         ...
         <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-200 hover:bg-slate-100 transition-colors">
            <label className="relative inline-flex items-center cursor-pointer select-none gap-2">
              ...
              />
              <span className="text-xs font-bold text-slate-500 uppercase">Debug Mode</span>
              <div className="w-9 h-5 bg-slate-300 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:left-[calc(100%-18px)] peer-checked:after:left-[calc(100%-20px)] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-slate-800"></div>
            </label>
         </div>

         {/* Import Button */}
         <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".json" className="hidden" />
         <button onClick={() => fileInputRef.current?.click()} className="text-xs font-bold text-slate-400 hover:text-indigo-600 flex items-center gap-1 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-200">
      ...
      <div className="pt-6 border-t border-slate-100 flex justify-end items-center">
          <button
            onClick={onNext}
            disabled={!isComplete}
            className={`px-8 py-4 rounded-xl font-bold text-lg shadow-lg transition-all flex items-center gap-3 ${
              isComplete
