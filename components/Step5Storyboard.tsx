...
const Step5Storyboard: React.FC<Props> = ({ script, stylePrompt, aspectRatio, assets, onBack }) => {
  ...
  const triggerSceneGeneration = async (sceneId: string) => {
    ...
    const relevantAssets = assets.filter(a => scene.description.toLowerCase().includes(a.name.toLowerCase()));
...
export default Step5Storyboard;